...
public class GuestDao {
    ...
    public List<Guest> findByFields(String fieldsToFind){
        ...
        String hql = "FROM Guest g JOIN g.address WHERE " +
                ...
                "OR g.passport.givenBy LIKE '%" + fieldsToFind + "%' OR to_char(g.passport.issuance,'DD-MM-YYYY') LIKE '%" +  fieldsToFind + "%' " +
                "OR cast(g.parking_lot_number as string) = '" + fieldsToFind + "' OR to_char(g.date_of_entry,'DD-MM-YYYY') LIKE '%" + fieldsToFind + "%' " +
                "OR to_char(g.departure_date,'DD-MM-YYYY') LIKE '%" + fieldsToFind + "%' OR to_char(g.birth_date,'DD-MM-YYYY') LIKE '%" + fieldsToFind + "%' " +
